...
atd_list = ["ProvingGroundIO","Parametric01","archinate","digitag","BjarkeIngels","etroxel","ColinMcCrone","iperezarnal"]
...
def get_all_tweets(screen_name):
	...
	outtweets = [[tweet.id_str, tweet.created_at, tweet.text.encode("latin1", 'ignore')] for tweet in alltweets]
	#write the csv
    # creates a separate file for each account in the atd_list
	with open('%s_tweets.csv' % screen_name, 'a', encoding='utf-8') as f: # open with 'w' writes and with 'a' it appends to existing file
		writer = csv.writer(f)
